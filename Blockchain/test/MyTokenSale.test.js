...
contract("TokenSale", async function (accounts) {
  ...
  it("there shouldnt be any coins in my account", async () => {
    ...
    return expect(instance.balanceOf.call(initialHolder)).to.eventually.be.a.bignumber.equal(
      new BN(0),
    );
  });
  ...
  it("should be possible to buy one token by simply sending ether to the smart contract", async () => {
    ...
    expect(
      tokenSaleInstance.sendTransaction({ from: recipient, value: web3.utils.toWei("1", "wei") }),
    ).to.be.fulfilled;
